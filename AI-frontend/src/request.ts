 ...
 * - 40100：未登录跳转登录页；若目标是已关模块则改走 /module-unavailable，禁止抢跳
 */
import axios from 'axios'
import { parseSafeJson } from '@/utils/safeJson'
import { capabilitySnapshot, snapshotEnabled } from '@/utils/capabilitySnapshot'
import { decide40100Action, moduleUnavailableLocation, moduleForPath } from '@/utils/moduleGate'
  ...
  function (response) {
    ...
    if (data?.code === 40100) {
      ...
      const action = decide40100Action({
        pathname,
        responseUrl: String(response.request?.responseURL || ''),
      ...
      if (action === 'unavailable') {
        if (!pathname.includes('/module-unavailable')) {
          ...
          console.warn('[api] 模块已关闭，跳过登录跳转')
          window.location.replace(moduleUnavailableLocation(pathname, requireModule))
        }
